function [sol_2] = ROB599_ControlsProject_part2_Team16(TestTrack,Xobs) %uncomment speed_cmd later!!
...
%Obstacle
hit_ctr=0;
%Find largest diagonal length of obstacle
for i=1:Nobs

   Xobs_xcenter(i,1)=mean(Xobs{i}(:,1));
   Xobs_ycenter(i,1)=mean(Xobs{i}(:,2));
   obs_diffs=[Xobs_xcenter(i,1)-Xobs{i}(:,1),Xobs_ycenter(i,1)-Xobs{i}(:,2)];
   max_obs_dist(i,1)= max(sqrt(sum(obs_diffs.^2,2)));
end
tic
...
%         new_setpoints(i,:,j-2) = new_setpoints(i,:,j-2)' + (TestTrack.bl(:,j-2) - TestTrack.cline(:,j-2)) * sideWeight/2;
%         new_setpoints(i,:,j-1) = new_setpoints(i,:,j-1)' + (TestTrack.bl(:,j-1) - TestTrack.cline(:,j-1)) * sideWeight;
%         new_setpoints(i,:,j) = new_setpoints(i,:,j)' + (TestTrack.bl(:,j) - TestTrack.cline(:,j)) * sideWeight;
%         new_setpoints(i,:,j+1) = new_setpoints(i,:,j+1)' + (TestTrack.bl(:,j+1) - TestTrack.cline(:,j+1)) * sideWeight;
%         new_setpoints(i,:,j+2) = new_setpoints(i,:,j+2)' + (TestTrack.bl(:,j+2) - TestTrack.cline(:,j+2)) * sideWeight/2;        
%     else
%         new_setpoints(i,:,j-2) = new_setpoints(i,:,j-2)' + (TestTrack.br(:,j-2) - TestTrack.cline(:,j-2)) * sideWeight/2;
%         new_setpoints(i,:,j-1) = new_setpoints(i,:,j-1)' + (TestTrack.br(:,j-1) - TestTrack.cline(:,j-1)) * sideWeight;
%         new_setpoints(i,:,j) = new_setpoints(i,:,j)' + (TestTrack.br(:,j) - TestTrack.cline(:,j)) * sideWeight;
%         new_setpoints(i,:,j+1) = new_setpoints(i,:,j+1)' + (TestTrack.br(:,j+1) - TestTrack.cline(:,j+1)) * sideWeight;
%         new_setpoints(i,:,j+2) = new_setpoints(i,:,j+2)' + (TestTrack.br(:,j+2) - TestTrack.cline(:,j+2)) * sideWeight/2; 
%     end    
    ...
    if(left)
    %if( norm(center_pt - TestTrack.bl(:,index1)) < norm(center_pt - TestTrack.br(:,index1)) )
        setpoints(:,j-2) = setpoints(:,j-2) + (TestTrack.bl(:,j-2) - TestTrack.cline(:,j-2)) * sideWeight/2;
        setpoints(:,j-1) = setpoints(:,j-1) + (TestTrack.bl(:,j-1) - TestTrack.cline(:,j-1)) * sideWeight*(2/3);
        setpoints(:,j) = setpoints(:,j) + (TestTrack.bl(:,j) - TestTrack.cline(:,j)) * sideWeight;
        setpoints(:,j+1) = setpoints(:,j+1) + (TestTrack.bl(:,j+1) - TestTrack.cline(:,j+1)) * sideWeight*(2/3);
        setpoints(:,j+2) = setpoints(:,j+2) + (TestTrack.bl(:,j+2) - TestTrack.cline(:,j+2)) * sideWeight/2;
    else
        setpoints(:,j-2) = setpoints(:,j-2) + (TestTrack.br(:,j-2) - TestTrack.cline(:,j-2)) * sideWeight/2;
        setpoints(:,j-1) = setpoints(:,j-1) + (TestTrack.br(:,j-1) - TestTrack.cline(:,j-1)) * sideWeight*(2/3);
        setpoints(:,j) = setpoints(:,j) + (TestTrack.br(:,j) - TestTrack.cline(:,j)) * sideWeight;
        setpoints(:,j+1) = setpoints(:,j+1) + (TestTrack.br(:,j+1) - TestTrack.cline(:,j+1)) * sideWeight*(2/3);
        setpoints(:,j+2) = setpoints(:,j+2) + (TestTrack.br(:,j+2) - TestTrack.cline(:,j+2)) * sideWeight/2;
    end   
...
%for i = 1:Nobs
    for j = 1:246
        setpoints(:,j) = setpoints(:,j) - [5*i/246 0]';
        tol = [15 15]';
        left_bound = setpoints(:,j) - TestTrack.bl(:,j) - tol;
        right_bound = setpoints(:,j) - TestTrack.br(:,j) - tol;
        if (~all(left_bound)) % (~all(new_setpoints(i,:,j) - TestTrack.bl(:,j)))
            setpoints(:,j) = 0.6 * (setpoints(:,j) + TestTrack.bl(:,j)); %new_setpoints(i,:,j) = 0.9 * (new_setpoints(i,:,j) + TestTrack.bl(:,j));
        end
        if (~all(right_bound)) % (~all(new_setpoints(i,:,j) - TestTrack.br(:,j)))
            setpoints(:,j) = 0.6 * (setpoints(:,j) + TestTrack.br(:,j)); %new_setpoints(i,:,j) = 0.9 * (new_setpoints(i,:,j) + TestTrack.br(:,j));
        end
    end
%end
...
while(1)
    ...
    pos=[x(1,1),x(1,3)];
    pos_obs_diffs=[pos(1)-Xobs_xcenter,pos(2)-Xobs_ycenter];
    pos_obs_dist=sqrt(sum(pos_obs_diffs.^2,2));
    if any((max_obs_dist-pos_obs_dist)>0)
    hit_ctr=hit_ctr+1;
    end
    
    i = i+1;
    elapsed=toc;
    if elapsed>20*60%300
        break;
    end
end
